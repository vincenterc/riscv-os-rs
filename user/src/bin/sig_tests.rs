...
use user_lib::{
    SIGCONT, SIGKILL, SIGSTOP, SIGUSR1, SignalAction, SignalFlags, close, exit, fork, getpid, kill,
    pipe, read, sigaction, sigprocmask, sigreturn, sleep, wait, write,
};
...
fn user_sig_test_failsignum() {
    ...
    if sigaction(50, Some(&new), Some(&mut old)) >= 0 {
        panic!("Wrong sigaction but succeeded!");
    }
}
